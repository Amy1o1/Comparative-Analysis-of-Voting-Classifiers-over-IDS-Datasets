...
all_ds = ["KDDCup", "NF-UQ-NIDS", "CIC-DDOS"]
...
file_name = sel_ds+"performance_records.csv"
...
def load_ds_for_display(sel_ds):
    ...
    if sel_ds == "NF-UQ-NIDS":
        ds_df = pd.read_csv('NF-UQ-NIDS_preprocessed.csv', header=None)
        ds = pd.read_csv('NF-UQ-NIDS_preprocessed.csv', header=None)
    if sel_ds == "CIC-DDOS":
        ds_df = pd.read_csv('cicDDOS_preprocessed.csv', header=None)
        ds = pd.read_csv('cicDDOS_preprocessed.csv', header=None)
    st.write(ds_df)
...
def load_ds_for_model(sel_ds):
    ...
    if sel_ds == "NF-UQ-NIDS":
        df = pd.read_csv("NF-UQ-NIDS_preprocessed.csv")
    if sel_ds == "CIC-DDOS":
        df = pd.read_csv("cicDDOS_preprocessed.csv")
    return df
...
with tab1:
    ...
    if sel_ds == "NF-UQ-NIDS":
        y = ds_for_model['Label']
        X = ds_for_model.drop('Label', axis=1)
    if sel_ds == "CIC-DDOS":
        y = ds_for_model['Class']
        X = ds_for_model.drop('Class', axis=1)
    for b in b_e:
...
if train == True:
    ...
    if sel_ds == "NF-UQ-NIDS":
        X_train, X_test, Y_train, Y_test = train_test_split(ds_for_model.drop('Label', axis=1), ds_for_model['Label'], stratify=ds_for_model['Label'], train_size=size, random_state=123)
    if sel_ds == "CIC-DDOS":
        X_train, X_test, Y_train, Y_test = train_test_split(ds_for_model.drop('Class', axis=1), ds_for_model['Class'], stratify=ds_for_model['Class'], train_size=size, random_state=123)    
...
if os.path.exists('./'+file_name):
    d = pd.read_csv(sel_ds+'performance_records.csv')
    with tab3:        
